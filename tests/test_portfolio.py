...
class TestPortfolio(unittest.TestCase):
  ...
  def test_buy(self):
    ...
    self.assertEqual(('2023-12-23', 'BTC', 40000, 0.125, 5000, 0, -5000, 0, 0, 5000),
    ...
    self.assertEqual(('2023-12-24', 'ETH', 5000, 0.75, 3750, 0, -3750, 0, 0, 1250),
                     tuple(self.portfolio.transactions.loc[1]))
  ...
  def test_buy_with_percentage_of_cash(self):
    ...
    self.assertEqual(('2023-12-24', 'BTC', 40000, 0.0625, 2500, 0, -2500, 0, 0, 2500),
                     tuple(self.portfolio.transactions.iloc[-1]))

    self.portfolio.buy('BTC', '2023-12-25', 40000, percentage_of_cash=100)
    self.assertEqual(('2023-12-25', 'BTC', 40000, 0.0625, 2500, 0, -2500, 0, 0, 0),
                     tuple(self.portfolio.transactions.iloc[-1]))
  ...
  def test_coinbase_buy(self):
    ...
    self.assertEqual(('2023-12-23', 'BTC', 40000, 0.125, 5000, 30, -5030, 0, 0, 4970),
    ...
    self.assertEqual(('2023-12-24', 'ETH', 5000, 0.75, 3750, 22.5, -3772.5, 0, 0, 1197.5),
  ...
  def test_sell_with_percentage_of_shares(self):
    ...
    self.assertEqual(('2023-12-24', 'BTC', 40000, -0.0625, 2500, 0, 2500, 0, 1, 7500),
                     tuple(self.portfolio.transactions.iloc[-1]))
    self.assertEqual(self.portfolio.cash(), 7500)

    self.portfolio.sell('BTC', '2023-12-25', 40000, percentage_of_shares=100)
    self.assertEqual(('2023-12-25', 'BTC', 40000, -0.0625, 2500, 0, 2500, 0, 2, 10000),
                     tuple(self.portfolio.transactions.iloc[-1]))
  ...
  def test_buys_and_sells(self):
    ...
    self.assertEqual([
      (0, ['2023-12-23', 'BTC', 35000, 0.1125, 3937.5, 23.625, -3961.125, 0, 0, 6038.875]),
      (1, ['2023-12-24', 'BTC', 40000, 0.1125, 4500.0, 27.0, -4527.0, 0, 0, 1511.875]),
      (3, ['2023-12-26', 'BTC', 40500, 0.1125, 4556.25, 27.3375, -4583.5875, 0, 0, 1513.1125]),
    ], list(zip(buys.index, buys.values.tolist())))

    sells = self.coinbase_portfolio.sells()

    self.assertEqual([
      (2, ['2023-12-25', 'BTC', 41000, -0.1125, 4612.5, 27.675, 4584.825, 363.2273, 1, 6096.7]),
      (4, ['2023-12-27', 'BTC', 42000, -0.1125, 4725.0, 28.35, 4696.65, 165.4685, 2, 6209.7625]),
      (5, ['2023-12-28', 'BTC', 43000, -0.1125, 4837.5, 29.025, 4808.475, 249.1495, 3, 11018.2375]),
    ...
    self.assertEqual([
      ...
      (1, ['2023-12-24', 'BTC', 40000, 0.1125, 4500.0, 27.0, -4527.0, 0, 0, 1511.875]),
      (3, ['2023-12-26', 'BTC', 40500, 0.1125, 4556.25, 27.3375, -4583.5875, 0, 0, 1513.1125]),
      (6, ['2023-12-28', 'ETH', 5000, 1.0, 5000.0, 30.0, -5030.0, 0, 0, 5988.2375]),
    ], list(zip(buys.index, buys.values.tolist())))
    ...
    self.assertEqual([
      ...
      (4, ['2023-12-27', 'BTC', 42000, -0.1125, 4725.0, 28.35, 4696.65, 165.4685, 2, 6209.7625]),
      (5, ['2023-12-28', 'BTC', 43000, -0.1125, 4837.5, 29.025, 4808.475, 249.1495, 3, 11018.2375]),
      (7, ['2023-12-29', 'ETH', 6000, -1.0, 6000.0, 36.0, 5964.0, 934.0, 1, 11952.2375])
    ], list(zip(sells.index, sells.values.tolist())))
  ...
  def test_wins_and_losses(self):
    ...
    self.assertEqual([
      (1, ['2023-12-24', 'BTC', 45000, -0.125, 5625.0, 33.75, 5591.25, 587.5, 1, 50561.25]),
      (5, ['2023-12-28', 'BTC', 46000, -0.125, 5750.0, 34.5, 5715.5, 711.75, 1, 50565.5])],
      list(zip(wins.index, wins.values.tolist())))

    losses = self.coinbase_portfolio.losses()

    self.assertEqual([
      (3, ['2023-12-26', 'BTC', 35000, -0.125, 4375.0, 26.25, 4348.75, -655.0, 1, 49880])],
